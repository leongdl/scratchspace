...
$3DS_MAX_INSTALLER_ZIP_S3_URI = "s3://your-bucket-name/path/to/3dsMax2025.zip"
$MAX_VERSION = "2025"
...
function Install-3dsMax {
    ...
    if (-not (Test-Path "$setupDir\3dsmax.zip")) {
        aws s3 cp --no-progress "$3DS_MAX_INSTALLER_ZIP_S3_URI" "$setupDir\3dsmax.zip"
...
# Adaptor install/upgrade runs EVERY boot (design: adaptor freshness). The install
# lands in Max's bundled Python which lives on the volume via the junction, so warm
# boots only pay a pip no-op/upgrade. Scripts dir is already on the machine Path.
function Install-Adaptor {
    # pip/ensurepip emit WARNINGs on stderr ("scripts ... not on PATH"); like reg.exe,
    # under strict mode those become terminating errors. Route via cmd, discard streams.
