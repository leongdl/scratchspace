<#
Bootstrap host configuration for AWS Deadline Cloud SMF Windows fleets.
The inline hostConfiguration.scriptBody limit is 15,000 characters; this stub
(under 1,000) downloads the real, unconstrained script from S3 and runs it.
Fleet role needs s3:GetObject on the script object.
#>
$ErrorActionPreference = "Stop"
trap { Write-Output "BOOTSTRAP ERROR: $($_.Exception.Message)"; exit 1 }

$SCRIPT_S3_URI = "s3://leongdldevbucket/DeadlineCloud/pv-cache/3dsmax-2025/host-config.ps1"
