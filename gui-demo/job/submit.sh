#!/bin/bash
# Submit the DCV Desktop job to Deadline Cloud
...
DOCKER_REPO="${DOCKER_REPO:-desktop-demo-dcv}"
...
echo "Submitting DCV Desktop job..."
echo "Farm: $FARM_ID"
echo "Queue: $QUEUE_ID"
...
python3 -m deadline bundle submit . \
    --farm-id "$FARM_ID" \
    --queue-id "$QUEUE_ID" \
    --max-retries-per-task 1 \
    --name "Rocky-DCV-Desktop" \
    --yes \
